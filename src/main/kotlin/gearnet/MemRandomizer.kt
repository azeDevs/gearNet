...
class MemRandomizer : XrdApi {
    ...
    private fun botJoinLobby() {
        ...
        val displayName = NameGen.getRandomName()
        botLobby[botId] = FighterData(botId, displayName, Random.nextInt(25).toByte(), 7, 0, 0, 0, 0)
        println("B: $displayName [${NameGen.getIdString(botId)}] has joined the lobby")
    }
    ...
    private fun botLeaveLobby() {
        ...
        println("B: ${s.userName} [${NameGen.getIdString(s.steamId)}] has left the lobby")
    }
    ...
    private fun botChangeLocation() {
        ...
        if (seatedBots.isNotEmpty()) {
            ...
            println("B: ${s.userName} [${NameGen.getIdString(s.steamId)}] has spotted a seated bot...${sightedBot.userName} [${NameGen.getIdString(sightedBot.steamId)}]")
...
//                ).getCabinetString()}, spot ${Player(sightedBot).getPlaySideString()}")
    ...
    private fun botChangeCharacter() {
        ...
        botLobby[t.steamId] = t
        println("B: ${s.userName} [${NameGen.getIdString(s.steamId)}] changed characters from ${getCharacterInitials(s.characterId)} to ${getCharacterInitials(t.characterId)}")
    }
