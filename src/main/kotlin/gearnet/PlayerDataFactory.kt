...
import gearnet.GearNetUpdates.Companion.IC_DATA_CHANGE
...
//                    oldData.guardGauge-oldPlayerData.guardGauge  // guardGaugeDelta
    ...
    fun receivedUpdates(): List<GNLog> {
        ...
        return if (updates.isNotEmpty()) {
            val totalUpdates = mutableListOf(GNLog(IC_DATA_PLAYER, "Player ${newData.userName} [${NameGen.getIdString(newData.steamId)}] got ${updates.size} ${plural("update", updates.size)}"))
