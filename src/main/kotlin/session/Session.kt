...
class Session: Controller() {
    ...
    fun updatePlayers(): Boolean {
        ...
        playerData.forEach { data ->
            if (data.steamUserId != 0L) {
                ...
                if (!players.containsKey(data.steamUserId)) {
                    ...
                    log("S: New player ID ${data.steamUserId} found ... (${data.displayName})")
                }
        ...
        playerData.forEach { data ->
            if (data.loadingPct in 1..99) {
                if (data.cabinetLoc.toInt() == 0 && data.playerSide.toInt() == 0) matchPlayersPending.p1 = data
                if (data.cabinetLoc.toInt() == 0 && data.playerSide.toInt() == 1) matchPlayersPending.p2 = data
            }
        }
        if (match.matchId == -1L && matchPlayersPending.p1.steamUserId > 0L && matchPlayersPending.p2.steamUserId > 0L) {
            match = Match(matches.size.toLong(), 0x0, matchPlayersPending)
            log("S: Generated Match ID ${matches.size.toLong()}")
    ...
    private fun resolveTheWinner(data: PlayerData, loserChange: Int) {
        players.values.filter { it.getSteamId() == data.steamUserId && it.hasWon() }.forEach { w ->
            ...
            if (match.getWinner() > -1 ) {
                ...
                log("S: Archived Match ID ${match.matchId} ... Snapshots ${match.allData().size}")
            }

            w.changeChain(1)
            val payout = w.getChain() * w.getMatchesWon() + w.getMatchesPlayed() + loserChange + (w.getChain() * w.getChain() * 100)
            w.changeBounty(payout)
            log("S: Player ID ${w.getSteamId()} won ${w.getChangeString()} with ${w.getBountyFormatted()} total ... (${data.displayName})")
    ...
    private fun resolveEveryoneElse(data: PlayerData): Int {
        ...
        players.values.filter { it.getSteamId() == data.steamUserId && it.hasLost() }.forEach { l ->
            ...
            log("S: Player ID ${l.getSteamId()} lost ${l.getChangeString()} with ${l.getBountyFormatted()} remaining ... (${data.displayName})")
            return loserChange
        }
        return 0
    }
    ...
    fun log(text:String) {
        if (consoleLog.size>55) consoleLog.removeAt(0)
        consoleLog.add(text)
        println(text)
    }
