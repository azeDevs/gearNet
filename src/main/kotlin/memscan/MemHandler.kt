...
class MemHandler : XrdApi {
    ...
    override fun getMatchSnap(): MatchSnap {
        ...
        try {
            ...
            val strikeStun = Pair(getByteBuffFromAddr(p1offs, 4)!!.int == 1, getByteBuffFromAddr(p2offs, 4)!!.int == 1)
            p1offs[1] = sortedStructOffs[2]
            p2offs[1] = sortedStructOffs[2]
            val canBurst = Pair(getByteBuffFromAddr(p1offs, 4)!!.int == 1, getByteBuffFromAddr(p2offs, 4)!!.int == 1)
            ...
            val stunMaximum = Pair(getByteBuffFromAddr(p1offs, 4)!!.int * 100, getByteBuffFromAddr(p2offs, 4)!!.int * 100)
            val timer = getByteBuffFromAddr(timeroffs, 4)!!.int
            val rounds = Pair(getByteBuffFromAddr(p1roundoffset, 4)!!.int, getByteBuffFromAddr(p2roundoffset, 4)!!.int)
            return MatchSnap(timer, health, rounds, tension, canBurst, strikeStun, guardGauge, stunProgress, stunMaximum)
        } catch (e: NullPointerException) {
            return MatchSnap()
        }
    }
